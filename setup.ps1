...
Write-Setup '      （build-dist.ps1 依赖本机路径 D:\DSH-desktop / D:\7-Zip）'
Write-Setup '  - 最终用户安装包：installer\setup.bat（会联网下载 Harness 引擎）'
Write-Setup '  - Harness 引擎无需手动准备：应用内点击「启动 Harness」自动探测 npx 缓存/npm 全局/'
Write-Setup '    %LOCALAPPDATA%\DSH\harness 源码目录，找不到则自动获取；也可用环境变量 DSH_HARNESS_DIR 显式指定'
